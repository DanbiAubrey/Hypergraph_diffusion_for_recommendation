...
'''
python main.py --model=HGNN_HD3 --dataset=lastfm  --lrate=0.0001 --weight_decay=5e-6 --drop_rate=0.2 --p=0.3 --cl_rate=1e-05 --temp=0.2 --reg=0.1 --early_stopping_steps=20 --seed=20 --mode=local_only
python main.py --model=HGNN_HD3 --dataset=amazon_books  --lrate=0.01 --weight_decay=5e-6 --drop_rate=0.2 --p=0.3 --cl_rate=1e-05 --temp=0.2 --reg=0.001 --early_stopping_steps=20 --seed=20
...
class HGNN_HD3(GraphRecommender):
    def __init__(self, conf, training_set, test_set, knowledge_set, **kwargs):
        super(HGNN_HD3, self).__init__(conf, training_set, test_set, knowledge_set, **kwargs)
...
class HGNNModel(nn.Module):
    def __init__(self, data, args, device):
        ...
        self.hgnn_layer_group = GroupAwareEncoder(self.data, self.p, self.drop_rate, self.layers)
    ...
    def calculate_local_embeddings(self, keep_rate:float=1):
        ...
        user_all_embeddings, item_all_embeddings = self.hgnn_layer_local(ego_embeddings)
    ...
    def calculate_group_embeddings(self, keep_rate:float=1):
        ...
        ego_embeddings = torch.cat([uEmbed, iEmbed], 0).to(device)
        sparse_norm_adj = self.edgeDropper(self.sparse_norm_adj, keep_rate)
        user_all_embeddings, item_all_embeddings = self.hgnn_layer_group(ego_embeddings)
        return user_all_embeddings, item_all_embeddings
    ...
    def forward(self, ego_embeddings):
        ...
        for k in range(self.layers):
            if k != self.layers - 1: 
                ...
                ego_embeddings = self.lns[k](self.edhnn_layers[0](ego_embeddings, self.sparse_norm_adj, self.edhnn_ui_n)) + res
            else:
                ego_embeddings = self.lns[k](self.edhnn_layers[1](ego_embeddings, self.sparse_norm_adj, self.edhnn_ui_n, act=False)) + res
...
class GroupAwareEncoder(nn.Module):
    def __init__(self, data, p, drop_rate, layers):

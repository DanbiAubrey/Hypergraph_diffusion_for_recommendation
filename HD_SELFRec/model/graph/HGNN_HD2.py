'''
KHGRec variation version1
- replaced HGConv(in SelfawareEncoder) with Hypergraph Diffusion
- python main.py --model=HGNN_HD2 --dataset=lastfm  --lrate=0.01 --weight_decay=5e-6 --drop_rate=0.2 --p=0.3 --cl_rate=1e-05 --temp=0.2 --reg=0.2 --early_stopping_steps=20  --seed=20 --mode=woglobal
- python main.py --model=HGNN_HD2 --dataset=ml-1m  --lrate=0.01 --weight_decay=5e-6 --drop_rate=0.2 --p=0.3 --cl_rate=1e-05 --temp=0.2 --reg=0.1 --early_stopping_steps=20  --seed=20 --mode=woglobal
'''
import time
...
from model.layers.layers4 import EquivSetGNN4
...
class HGNN_HD2(GraphRecommender):
...
class SelfAwareEncoder(nn.Module):
    def __init__(self, data, emb_size, hyper_size, n_layers, leaky, drop_rate, device, use_self_att=False):
        ...
        self.edhnn_layers = nn.ModuleList([EquivSetGNN4.EquivSetGNN4(hyper_size, self.edhnn_args, self.sparse_norm_adj, self.data) for i in range(2)])
        ''' ------- '''
        ...
        ''' my code '''
        self.edhnn_user_n = self.data.n_users # norm_adj:[u_n +i_n, u_n +i_n]
        self.edhnn_item_n = self.data.n_items
    ...
    def forward(self, ego_embeddings, sparse_norm_adj):
        ...
        for k in range(self.layers):
            ...
            if k != self.layers - 1: 
                ...
                ego_embeddings = self.edhnn_layers[0](ego_embeddings, self.sparse_norm_adj, self.edhnn_ui_n) + res
            else:
                ego_embeddings = self.edhnn_layers[1](ego_embeddings, self.sparse_norm_adj, self.edhnn_ui_n) + res

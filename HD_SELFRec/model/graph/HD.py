...
python main.py --model=HD --dataset=ml-1m  --lrate=0.01 --weight_decay=5e-6 --drop_rate=0.2 --p=0.3 --cl_rate=1e-05 --temp=0.2 --reg=0.1 --early_stopping_steps=20  --seed=20 --mode=woglobal
'''
import time
...
class SelfAwareEncoder(nn.Module):
    def __init__(self, data, emb_size, hyper_size, n_layers, leaky, drop_rate, device, use_self_att=False):
        ...
        self.edhnn_layers = nn.ModuleList([EquivSetGNN2.EquivSetGNN(hyper_size, self.edhnn_args, self.sparse_norm_adj, self.data) for i in range(2)])
        ''' ------- '''
    ...
    def forward(self, ego_embeddings, sparse_norm_adj):
        ...
        for k in range(self.layers):
            ...
            # residual connection required for convergence
            if k != self.layers - 1: 

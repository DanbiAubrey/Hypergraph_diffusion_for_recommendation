...
class WaveletHD(GraphRecommender):
    ...
    def train(self, load_pretrained=False):
        ...
        for ep in range(self.maxEpoch):        
            ...
            lst_kg_losses.append([ep,0])
            lst_cl_losses.append([ep, 0]) 
...
#         self.edhnn_layers = EquivSetGNN.EquivSetGNN(hyper_size, self.edhnn_args)
#         ''' ------- '''
...
#         u_embeddings = ego_embeddings[:self.data.n_users]
#         i_embeddings = ego_embeddings[self.data.n_users:]
...
#                 u_embeddings = self.edhnn_layers(u_embeddings, self.hyper_uu, self.edhnn_user_n)
#                 i_embeddings = self.edhnn_layers(i_embeddings, self.hyper_ii, self.edhnn_item_n)
#             else:
#                 #ego_embeddings = self.lns[k](self.hgnn_layers[k](sparse_norm_adj, ego_embeddings, act=False)) + res
#                 u_embeddings = self.edhnn_layers(u_embeddings, self.hyper_uu, self.edhnn_user_n) 
#                 i_embeddings = self.edhnn_layers(i_embeddings, self.hyper_ii, self.edhnn_item_n) 
            
#             ego_embeddings = torch.cat((u_embeddings, i_embeddings), 0)
...
class SelfAwareEncoder(nn.Module):
    ...
    def forward(self, ego_embeddings, sparse_norm_adj):
        ...
        for k in range(self.layers):
            ...
            if k != self.layers - 1: 
                #ego_embeddings = self.lns[k](self.hgnn_layers[k](sparse_norm_adj, ego_embeddings))  + res
                ego_embeddings = self.edhnn_layers(ego_embeddings, self.dense_hypergraph, self.edhnn_ui_n)
            else:
                ego_embeddings = self.edhnn_layers(ego_embeddings, self.dense_hypergraph, self.edhnn_ui_n)

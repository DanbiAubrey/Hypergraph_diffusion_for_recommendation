...
class EquivSetConv4(nn.Module):
    def __init__(self, in_features, out_features, mlp1_layers=1, mlp2_layers=1,
        ...
        self.hwnn_layers = nn.ModuleList([wavelet.HWNN(self.hwnn_args['filters'], self.hwnn_args['dropout'], self.hwnn_args['ncount'], self.hwnn_args['feature_number'], self.device, self.data) for i in range(2)])
        self.hgcn_layer = HGCNConv(0.2)
